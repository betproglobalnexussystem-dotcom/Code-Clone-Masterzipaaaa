...
export default function MatchRow({ match, onAddBet, betSelections, onMatchClick }: MatchRowProps) {
  ...
  const dcArr = [
    { label: "1X", pick: `${match.homeTeam} or Draw`, odd: dc?.oneX ?? 0, key: "1x" },
    { label: "X2", pick: `Draw or ${match.awayTeam}`, odd: dc?.xTwo ?? 0, key: "x2" },
    { label: "12", pick: `${match.homeTeam} or ${match.awayTeam}`, odd: dc?.oneTwo ?? 0, key: "12" },
  ];
